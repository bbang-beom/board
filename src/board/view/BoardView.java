...
public class BoardView {
	...
	public void displayMenu() {
		...
		System.out.println(
				"-------------------------------------------------------------------------------------------------------------------------------------------------");
		System.out.println(
				"| 0. 종료 | 1. 게시물 작성 | 2. 게시물 목록 | 3. 게시물 내용 보기 | 4. 게시물 수정 | 5. 게시물 삭제 | 6. 조회 순 정렬 | 7. 추천 순 정렬 | | 8. 최신 순 정렬 | 9. 파일 저장 |");
		System.out.println(
				"-------------------------------------------------------------------------------------------------------------------------------------------------");
	}
				...
				if (option == 1) { // 전체 보기
					...
					for (BoardDTO data : datas) { // 리스트를 돌면서
						if (data.getTitle().contains(search)) { // 검색어가 포함된 게시물만 출력
							System.out.println(
									"| " + data.getIndex() + "\t| " + data.getName() + "\t| " + data.getTitle() + "\t| "
											+ data.getViewCount() + "\t|" + date.format(data.getDate()) + "\t|");
							flag = true;
						}
					}
	...
	public void displayContent(BoardDTO boardData) {
		if (boardData != null) {
			...
			System.out.println("| " + boardData.getIndex() + "\t| " + boardData.getName() + "\t| "
					+ boardData.getTitle() + "\t| " + boardData.getContent() + "\t| " + boardData.getViewCount() + "\t|"
					+ date.format(boardData.getDate()) + "\t|");
		} else {
			notExist();
		}
	}
